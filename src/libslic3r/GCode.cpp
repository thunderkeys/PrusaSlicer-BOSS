            ...
            if (radius == 0) {
                ...
                if (const double line_length = (p - prev).norm(); line_length > 0) {
                    ...
                    if (m_small_area_infill_flow_compensator) {
                        ...
                        extrusion_amount = m_small_area_infill_flow_compensator->modify_flow(line_length, extrusion_amount, path_attr.role);

                        if (m_config.gcode_comments && old_extrusion_amount > 0 && !Slic3r::nearly_equal(extrusion_amount, old_extrusion_amount)) {
                            tempComment += Slic3r::format(" | Old Flow Value: %0.5f Length: %0.5f", old_extrusion_amount, line_length);
                        }
                    }
